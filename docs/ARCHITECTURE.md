...
This document describes the architecture of the .NET MAUI SkiaSharp KumikoUI for contributors and maintainers.
...
KumikoUI/
├── src/
│   ├── KumikoUI.Core/         # Platform-independent core (net9.0)
...
│   ├── KumikoUI.SkiaSharp/    # SkiaSharp backend (net9.0)
│   └── KumikoUI.Maui/         # .NET MAUI integration (net10.0 multi-target)
│       ├── Hosting/             # Service registration extensions
│       └── Platforms/           # Per-platform keyboard handling
├── samples/
│   └── SampleApp.Maui/         # Demo application
├── tests/
│   └── KumikoUI.Core.Tests/  # Unit tests
...
KumikoUI.SkiaSharp (net9.0 → SkiaSharp 3.119.2)
       ↑
KumikoUI.Maui (net10.0-ios/android/maccatalyst → MAUI + SkiaSharp.Views.Maui.Controls)
```

**KumikoUI.Core** has no dependency on any rendering or UI framework. All rendering operations go through the `IDrawingContext` abstraction, making the core portable to any backend (SkiaSharp, Direct2D, CoreGraphics, HTML Canvas, etc.).
...
|-------|---------------|
| `DataGridRenderer` | Orchestrates the entire drawing pipeline — layout, painting, overlays |
| `DataGridSource` | Data management — filtering, sorting, grouping, summaries, change observation |
| `GridLayoutEngine` | Column width computation (Auto/Star/Fixed), visible row/column ranges |
| `GridHitTester` | Resolves pointer coordinates to one of 14 `HitRegion` types |
...
| `KumikoUIHostingExtensions` | `builder.UseSkiaKumikoUI()` service registration |
...
~60 `GridPaint` objects are pre-computed once per frame from `DataGridStyle`. Factory methods (`BackgroundPaint`, `CellTextPaint`) handle dynamic per-cell overrides without heap allocation during rendering.
...
`DataGridSource.BuildAccessor` compiles `Func<object, object?>` from dotted property paths (e.g., `"Address.City"`) via `Expression.Property` chains. Compiled delegates are cached by `"TypeName.PropertyPath"`.
...
2. Column `CellStyle`
3. Grid defaults from `DataGridStyle`

### O(1) Incremental Updates
`DataGridSource.TryIncrementalUpdate` handles simple `INotifyCollectionChanged` Add/Remove operations without a full `RebuildView()` when no sort, filter, group, or summary transforms are active.
...
14 `HitRegion` types (Cell, Header, ColumnResizeHandle, FilterIcon, SortIndicator, FrozenDivider, GroupPanel, GroupChip, RowDragHandle, TableSummary, GroupHeader, GroupSummary, FrozenRow, None) are resolved across 3 column panes by `GridHitTester`.
